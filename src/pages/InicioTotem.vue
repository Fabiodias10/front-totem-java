      ...
      <q-card style="max-width: 350px; width: 100%">
        ...
        <q-card-section class="q-pt-none">
          <q-uploader
            ...
            accept="image/*"
            :url="urlApiBackendFundoPersonalizado"
            field-name="arquivo"
            @uploaded="onUploadConcluido"
            @failed="onUploadErro"
            color="primary"
...
const urlApiBackendRestaurar = computed(() => {
  return `http://${totemStore.ipServidor}:9095/restaurar`;
});

const urlApiBackendMidiaEntrada = computed(() => {
  return `http://${totemStore.ipServidor}:9095/MidiaEntrada`;
});

const urlApiBackendMidiaSaida = computed(() => {
  return `http://${totemStore.ipServidor}:9095/MidiaSaida`;
});
